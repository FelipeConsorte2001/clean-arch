import { Entity } from '@/shared/doman/entities/entity'
...
describe('InMemoryRepository unit tests', () => {
  ...
  describe('search method', () => {
    it('should apply only pagination when other params are null', async () => {
      const entities = new StubEntity({ name: 'test', price: 50 })
      const items = Array(16).fill(entities)
      sut.items = items

      const params = await sut.search(new SearchParams())
      expect(params).toStrictEqual(
        new SearchResult({
          items: Array(15).fill(entities),
          total: 16,
          currentPage: 1,
          perPage: 15,
          sort: null,
